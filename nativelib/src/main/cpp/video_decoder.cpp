...
// 延迟恢复常量
// L1: 同步模式自适应 drain（浅层堆积→顺序渲染，深层堆积→跳到最新帧）
...
// 高帧率自适应：120Hz 下 VPU 管线延迟（15-25ms）本身就远超帧间隔，
// 需要更宽松的阈值避免误丢帧。
// 基准阈值（适用于 ≤60fps）
static constexpr double kL5LatencyRatio_Base = 2.5;   // 延迟 > 帧间隔 × 此值
static constexpr double kL5IntervalRatio_Base = 0.3;   // 输出间隔 < 帧间隔 × 此值
...
int VideoDecoder::SubmitDecodeUnit(const uint8_t* data, int size, 
    ...
    // 核心思路：在网络回调线程中尝试直接提交（timeout=0，不阻塞），失败则入队
    if (config_.decoderMode == DecoderMode::SYNC) {
        // 更新接收帧统计
        UpdateReceivedStats(size, hostProcessingLatency);
        ...
        // 尝试直接提交到解码器：timeout=0，仅一次
        // 参考官方文档：timeoutUs = 0 表示立即退出，不阻塞网络回调线程
        {
            std::shared_lock<std::shared_mutex> codecLock(codecMutex_);
            uint32_t inputIndex = 0;
            OH_AVErrCode ret = pfn_QueryInputBuffer(decoder_, &inputIndex, kSyncDirectSubmitTimeoutUs);
            ...
            // AV_ERR_TRY_AGAIN_LATER：解码器忙，静默回退到队列
        }
    ...
    if (enqueueTimeMs > 0) {
        ...
        if (lastAsyncRender > 0 && !isKeyframe &&
            ...
            if (outputInterval < static_cast<int64_t>(expectedFrameTimeMs * intervalRatio) &&
                ...
                instantDecodeTimeMs >= kL5AbsoluteLatencyFloorMs) {
                OH_VideoDecoder_FreeOutputBuffer(codec, index);
                {
                    std::lock_guard<std::mutex> lock(self->statsMutex_);
                    self->stats_.droppedFrames++;
    ...
    while (syncDecodeRunning_ && running_) {
        ...
        if (outputResult > 0) {
            ...
            if (pendingFrameQueue_.empty() && syncDecodeRunning_) {
                // 等待新帧到达或超时：半帧间隔（帧率自适应）
                // 60fps → 8333us, 120fps → 4167us
                int64_t halfFrameUs = static_cast<int64_t>(500000.0 / config_.fps);
                pendingFrameCond_.wait_for(lock, std::chrono::microseconds(halfFrameUs));
            }
        }
...
int VideoDecoder::SyncProcessOutput(int64_t timeoutUs) {
    ...
    // L1 自适应延迟恢复
    // 
    // 原理：取出所有已解码帧，根据堆积深度选择策略：
    //   - 浅层堆积 (≤ smoothThreshold)：按顺序全部渲染（平滑追赶）
    //     适用于 IDR 帧解码慢导致的 2-3 帧堆积，不计丢帧
    //   - 深层堆积 (> smoothThreshold)：仅渲染最新帧（快速恢复）
    //     适用于严重卡顿/网络抖动导致的大量堆积
    //
    // 效果：
    //   强设备：几乎不堆积 → 无影响
    //   弱设备：IDR 导致 2-3 帧堆积 → 按顺序渲染，Rd 不抖
    //   极弱设备：5+ 帧堆积 → drain-to-latest 快速恢复
    // ================================================================
    ...
    // 自适应阈值：高帧率允许更多平滑追赶（每帧时间更短）
    // 120fps 每帧 8.33ms：3 帧堆积 = 25ms 追赶，几乎无感
    // 60fps 每帧 16.67ms：2 帧堆积 = 33ms 追赶
    const int smoothThreshold = (config_.fps > 90) ? 3 : 2;
    
    int totalFrames = static_cast<int>(outputFrames.size());
    
    if (totalFrames <= smoothThreshold) {
        // === 浅层堆积：按顺序渲染所有帧（平滑追赶） ===
        // Surface 层会自动按 VSync 节奏逐帧呈现，视觉上是正常播放
        // 不计入丢帧统计，Rd fps 不会抖动
        for (int i = 0; i < totalFrames; i++) {
            auto& frame = outputFrames[i];
            int64_t framePts = frame.attr.pts;
            
            // 获取入队时间
            int64_t frameEnqueueTimeMs = 0;
            {
                std::lock_guard<std::mutex> lock(timestampMutex_);
                auto it = timestampToEnqueueTime_.find(framePts);
                if (it != timestampToEnqueueTime_.end()) {
                    frameEnqueueTimeMs = it->second;
                    timestampToEnqueueTime_.erase(it);
                }
            }
            
            // 更新统计
            UpdateDecodedStats(framePts, frameEnqueueTimeMs, frame.attr.flags);
            
            // 渲染（立即提交到 Surface）
            OH_AVErrCode renderRet = OH_VideoDecoder_RenderOutputBuffer(decoder_, frame.index);
        ...
        if (totalFrames > 1) {
            OH_LOG_INFO(LOG_APP, "L1 smooth catch-up: rendered %{public}d queued frames in sequence", totalFrames);
        }
        
        return 1;
    }
    
    // === 深层堆积：Drain-to-latest（仅渲染最新帧） ===
    ...
    OH_LOG_INFO(LOG_APP, "L1 drain-to-latest: skipped %{public}d frames (queue=%{public}d > threshold=%{public}d)",
                drainedCount, totalFrames, smoothThreshold);
