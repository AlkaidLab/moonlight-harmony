...
// 延迟恢复常量
// L1: 同步模式 drain-to-latest（始终生效，零开销）
...
void VideoDecoder::OnOutputBufferAvailable(OH_AVCodec* codec, uint32_t index,
    ...
    if (enqueueTimeMs > 0) {
        ...
        if (instantDecodeTimeMs > expectedFrameTimeMs * kAsyncSkipThresholdMultiplier &&
            ...
            // 更新瞬时延迟跟踪（即使跳过也要更新，用于 L3 判断）
            self->lastInstantDecodeTimeMs_.store(instantDecodeTimeMs);
            static int asyncSkipCount = 0;
            if (++asyncSkipCount % 30 == 1) {
                OH_LOG_INFO(LOG_APP, "Async latency recovery: skipped frame (decode=%{public}lldms, expected=%.1fms, total_skipped=%{public}d)",
                            static_cast<long long>(instantDecodeTimeMs), expectedFrameTimeMs, asyncSkipCount);
            }
            return;
        }
    }
...
int VideoDecoder::SyncProcessOutput(int64_t timeoutUs) {
    ...
    // ================================================================
    // L1 延迟恢复：Drain-to-latest（始终生效）
    // 原理与 Android Moonlight 一致：取出所有已解码帧，仅渲染最新一帧
    // 正常情况下每次只有 1 帧可用（零开销），延迟堆积时立即跳到最新帧
    // ================================================================
    int drainedCount = 0;
    while (running_ && syncDecodeRunning_) {
        ...
        {
            std::lock_guard<std::mutex> lock(timestampMutex_);
            timestampToEnqueueTime_.erase(attr.pts);
        }
    ...
    if (drainedCount > 0) {
        OH_LOG_INFO(LOG_APP, "Sync drain-to-latest: skipped %{public}d intermediate frames", drainedCount);
    }
