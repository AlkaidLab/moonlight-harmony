...
客户端在用户进入应用列表后机会式调用该接口，以较小流量估算 Sunshine 到客户端的端到端下行吞吐。探测不阻塞应用列表；点击游戏时复用同一路径的进行中任务，缓存未命中时最多等待 800 ms。探测失败或超时时静默回退到用户码率上限，串流建立后再由 ABR 校准。
...
客户端进入应用列表
  ...
  -> 按路径指纹缓存结果
  -> 用户点击游戏
  -> 缓存命中则立即启动，否则复用同路径任务并最多等待 800 ms
  -> 等待超时则取消本轮探测并回退
  -> 使用缓存结果计算初始码率
  -> 串流建立后由实际 UDP 指标和 ABR 校准
```

禁止在电脑列表的周期轮询中执行带宽探测。客户端普通离开应用列表或网络路径发生切换时，应立即取消未完成的请求；进入串流时保留同路径任务供启动流程复用。
...
- 累计墙钟时间达到 800 ms。
- 累计有效载荷达到服务端或客户端流量上限。
- 页面普通离开、网络切换、启动等待超过 800 ms 或请求被取消。
...
- 客户端在计费或蜂窝网络默认不执行主动探测，初始码率回退到用户上限并由运行期 ABR 调整。
...
1. 实现路径分类与探测结果缓存。
2. 在应用列表空闲窗口执行渐进探测。
3. 将结果接入初始码率策略，LAN/WAN/VPN 不直接决定码率。
